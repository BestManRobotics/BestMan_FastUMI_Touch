...
---------------------------------------
功能描述:
    本脚本演示如何使用 FastUMITouchArm SDK 进行关节空间的规划运动（movj）。
    通过 set_joint 方法，机械臂将根据指定的时间（tf）利用内部的多项式插值算法，
    平滑地从当前位姿移动到目标关节位姿。

核心 API 说明:
    - FastUMITouchArm(can_port): 初始化机械臂，自动加载模型参数与 CAN 接口。
...
print("FastUMITouch SDK Initialized")
